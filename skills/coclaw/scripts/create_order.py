#!/usr/bin/env python3
"""Create Coclaw marketplace order (OpenClaw purchase) automatically."""
...
DEFAULT_BUYER_WALLET = "GAXIVISOBDOMLXN6MPCTHKHSPC5W2JOUPE227ML4H7ZRTHY47YICIRDD"
TERMINAL_STATES = {"COMPLETED", "FAILED"}


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Create Coclaw order via OpenClaw adapter API"
    )
    parser.add_argument(
        "--listing-id", help="Listing ID. If omitted, auto-select first active listing."
    )
    parser.add_argument(
        "--buyer-wallet",
        default=DEFAULT_BUYER_WALLET,
        help="Buyer Stellar public key (G...)",
    )
    parser.add_argument(
        "--input-json",
    ...
    parser.add_argument(
        "--timeout-sec", type=int, default=180, help="Polling timeout seconds"
    )
    parser.add_argument(
        "--interval-sec", type=int, default=3, help="Polling interval seconds"
    )
...
def wait_purchase(
    base_url: str, purchase_id: str, timeout_sec: int, interval_sec: int
) -> dict[str, Any]:
