#!/usr/bin/env python3
"""Create Coclaw marketplace listing (service manifest) automatically."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Create Coclaw listing (service)")
    parser.add_argument(
        "--service-id", help="Service/listing id. If omitted, auto-generate."
    )
    parser.add_argument("--name", default="Auto Service", help="Service name")
    parser.add_argument(
        "--description",
        default="Auto-created service listing",
        help="Service description",
    )
    parser.add_argument("--price-usdt", default="1.0", help="Price in USDT string")
    parser.add_argument(
        "--supplier-wallet",
        default=DEFAULT_SUPPLIER_WALLET,
        help="Supplier Stellar public key (G...)",
    )
    parser.add_argument(
        "--input-schema-json",
    ...
    parser.add_argument(
        "--inactive", action="store_true", help="Create listing as inactive"
    )
    parser.add_argument(
        "--dry-run", action="store_true", help="Print payload without API call"
    )
...
def main() -> int:
    ...
    try:
        ...
        output_schema = parse_json_object(
            args.output_schema_json, "--output-schema-json"
        )
